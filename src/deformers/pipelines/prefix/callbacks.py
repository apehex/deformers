...
def format_state(state: dict) -> dict:
    ...
    return {
        'switch': f"[{' '.join(state['switch/test'] * ['test'] + state['switch/grad'] * ['grad'] + state['switch/log'] * ['log'] + state['switch/save'] * ['save'])}]",
        ...
        'test': f"(kld(k: {state['test/kld/k']:.6f}) topk(k: {state['test/topk/k']:.6f}))",
