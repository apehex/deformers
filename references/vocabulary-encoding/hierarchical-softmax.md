...
ArXiv: 1310.4546
Authors: Tomas Mikolov, Google Inc., Mountain View, Ilya Sutskever, Kai Chen, Greg Corrado, Jeffrey Dean
Sections: 12
...
alternative to the hierarchical softmax called negative sampling. An inherent limitation of word representations is their indifference
...
for learning word vectors, training of the Skip-gram model (see Figure [1](#S2.F1))
...
Figure: Figure 1: The Skip-gram model architecture. The training objective is to learn word vector representations that are good at predicting the nearby words.
Refer to caption: https://ar5iv.labs.arxiv.org/html/1310.4546/assets/x1.png
...
or a document.
More formally, given a sequence of training words $w_{1},w_{2},w_{3},\ldots,w_{T}$, the objective of the Skip-gram model is to maximize
...
of the center word $w_{t}$). Larger $c$ results in more
...
$p(w_{t+j}|w_{t})$ using the softmax function:

$$ $p(w_{O}|w_{I})=\frac{\exp\left({v^{\prime}_{w_{O}}}^{\top}v_{w_{I}}\right)}{\sum_{w=1}^{W}\exp\left({v^{\prime}_{w}}^{\top}v_{w_{I}}\right)}$ (2) $$

where $v_{w}$ and $v^{\prime}_{w}$ are the “input” and “output” vector representations
of $w$, and $W$ is the number of words in the vocabulary. This
formulation is impractical because the cost of computing $\nabla\log p(w_{O}|w_{I})$ is proportional to $W$, which is often large
($10^{5}$–$10^{7}$ terms).
...
the probability distribution, it is needed to evaluate only about $\log_{2}(W)$ nodes.
...
Then the hierarchical softmax defines $p(w_{O}|w_{I})$ as follows:

$$ $p(w|w_{I})=\prod_{j=1}^{L(w)-1}\sigma\left([\![n(w,j+1)=\mathrm{ch}(n(w,j))]\!]\cdot{v^{\prime}_{n(w,j)}}^{\top}v_{w_{I}}\right)$ (3) $$

where $\sigma(x)=1/(1+\exp(-x))$. It can be verified that
$\sum_{w=1}^{W}p(w|w_{I})=1$. This implies that
the cost of computing $\log p(w_{O}|w_{I})$ and $\nabla\log p(w_{O}|w_{I})$ is proportional to $L(w_{O})$, which on average is no greater
than $\log W$. Also, unlike the standard softmax formulation of the Skip-gram
which assigns two representations $v_{w}$ and $v^{\prime}_{w}$ to each word $w$, the
hierarchical softmax formulation has
one representation $v_{w}$ for each word $w$ and one representation $v^{\prime}_{n}$
...
Figure: Figure 2: Two-dimensional PCA projection of the 1000-dimensional Skip-gram vectors of countries and their capital cities. The figure illustrates ability of the model to automatically organize concepts and learn implicitly the relationships between them, as during the training we did not provide any supervised information about what a capital city means.
Refer to caption: https://ar5iv.labs.arxiv.org/html/1310.4546/assets/x2.png
...
$$ $\log\sigma({v^{\prime}_{w_{O}}}^{\top}v_{w_{I}})+\sum_{i=1}^{k}\mathbb{E}_{w_{i}\sim P_{n}(w)}\left[\log\sigma(-{v^{\prime}_{w_{i}}}^{\top}v_{w_{I}})\right]$ (4) $$

which is used to replace every $\log P(w_{O}|w_{I})$ term in the Skip-gram objective.
Thus the task is to distinguish the target word
$w_{O}$ from draws from the noise distribution $P_{n}(w)$ using logistic regression,
...
Both NCE and NEG have the noise distribution $P_{n}(w)$ as
a free parameter. We investigated a number of choices for $P_{n}(w)$
...
simple subsampling approach: each word $w_{i}$ in the training set is
discarded with probability computed by the formula

$$ $P(w_{i})=1-\sqrt{\frac{t}{f(w_{i})}}$ (5) $$

where $f(w_{i})$ is the frequency of word $w_{i}$ and $t$ is a chosen
...
the analogical reasoning task(^1^11code.google.com/p/word2vec/source/browse/trunk/questions-words.txt)
...
analogy test set is reported in Table [1](#S3.T1).
...
$$ $\textrm{score}(w_{i},w_{j})=\frac{\textrm{count}(w_{i}w_{j})-\delta}{\textrm{count}(w_{i})\times\textrm{count}(w_{j})}.$ (6) $$
...
reasoning task that involves phrases. Table [2](#S4.T2) shows
examples of the five categories of analogies used in this task. This dataset is publicly available
on the web(^2^22code.google.com/p/word2vec/source/browse/trunk/questions-phrases.txt).
...
of the frequent tokens. The results are summarized in Table [3](#S4.T3).
...
using various models. In Table [4](#S4.T4), we show a sample of such comparison.
...
This phenomenon is illustrated in Table [5](#S5.T5).
...
the web(^3^33http://metaoptimize.com/projects/wordreprs/). Mikolov et al. [8] have already evaluated these word representations on the word analogy task,
...
words in Table [6](#S6.T6). These examples show that the big Skip-gram model trained on a large
...
described in this paper available as an open-source project(^4^44code.google.com/p/word2vec).
...
PhD thesis, PhD Thesis, Brno University of Technology, 2012.
- [8]
