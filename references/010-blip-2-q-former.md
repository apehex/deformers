...
els.
BLIP-2 bridges the modality gap with a
...
capabilities of zero-shot image-to-text generation
that can follow natural language instructions.
1. Introduction
...
1Salesforce Research. Correspondence to: Junnan Li <jun-
nan.li@salesforce.com>.
...
2022; Chen et al., 2022a; Tiong et al., 2022; Guo et al.,
2022). The key challenge in using a frozen LLM is to
align visual features to the text space. To achieve this,
Frozen (Tsimpoukelli et al., 2021) finetunes an image en-
coder whose outputs are directly used as soft prompts for
the LLM. Flamingo (Alayrac et al., 2022) inserts new cross-
attention layers into the LLM to inject visual features, and
pre-trains the new layers on billions of image-text pairs.
...
BLIP-2: Bootstrapping Language-Image Pre-training with Frozen Image Encoders and Large Language Models
The audi e-tron quattro concept is a 
...
models for image captioning. In CVPR, pp. 18009–18019,
2022a.
...
Y., Kozareva, Z., and Zhang, Y. (eds.), EMNLP Findings,
2022.
