...
class TestEmbedMse:
    ...
    def test_zero_error_on_identical_tensors(self):
        ...
        assert deformers.pipelines.eval.embed_mse(__x, __x) == pytest.approx(0.0, abs=1e-6)

    def test_positive_on_different_tensors(self):
        __x = torch.zeros(_B, _T, 32)
        __y = torch.ones(_B, _T, 32)
        assert deformers.pipelines.eval.embed_mse(__x, __y) > 0.0

    def test_returns_float(self):
        __x = torch.randn(_B, _T, 32)
        __y = torch.randn(_B, _T, 32)
        assert isinstance(deformers.pipelines.eval.embed_mse(__x, __y), float)

class TestHiddenMse:

    def test_zero_error_on_identical_tensors(self):
        __x = torch.randn(_B, _T, 32)
        assert deformers.pipelines.eval.hidden_mse(__x, __x) == pytest.approx(0.0, abs=1e-6)

    def test_positive_on_different_tensors(self):
        __x = torch.zeros(_B, _T, 32)
        __y = torch.ones(_B, _T, 32)
        assert deformers.pipelines.eval.hidden_mse(__x, __y) > 0.0
...
class TestKlDivergence:
    ...
    def test_zero_on_identical_logits(self):
        ...
        __kl = deformers.pipelines.eval.kl_divergence(__x, __x)
    ...
    def test_positive_on_different_logits(self):
        ...
        assert deformers.pipelines.eval.kl_divergence(__t, __s) > 0.0

    def test_returns_float(self):
        __x = torch.randn(_B, _T, _V)
        assert isinstance(deformers.pipelines.eval.kl_divergence(__x, __x), float)

    def test_shape_mismatch_raises(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V + 1)
        with pytest.raises(AssertionError):
            deformers.pipelines.eval.kl_divergence(__t, __s)

    def test_finite_output(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __kl = deformers.pipelines.eval.kl_divergence(__t, __s)
        assert math.isfinite(__kl)

    def test_reduction_sum_differs_from_batchmean(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __kl_bm = deformers.pipelines.eval.kl_divergence(__t, __s, reduction='batchmean')
        __kl_sum = deformers.pipelines.eval.kl_divergence(__t, __s, reduction='sum')
...
class TestTop1MatchRate:
    ...
    def test_rate_one_on_identical_logits(self):
        __x = torch.randn(_B, _T, _V)
        assert deformers.pipelines.eval.top1_match_rate(__x, __x) == pytest.approx(1.0)
    ...
    def test_rate_zero_when_all_mismatch(self):
        ...
        __s = torch.zeros(_B, _T, _V)
        __s[:, :, 1] = 1.0
        assert deformers.pipelines.eval.top1_match_rate(__t, __s) == pytest.approx(0.0)
    ...
    def test_partial_match(self):
        ...
        assert deformers.pipelines.eval.top1_match_rate(__t, __s) == pytest.approx(0.5)

    def test_returns_float(self):
        __x = torch.randn(_B, _T, _V)
        assert isinstance(deformers.pipelines.eval.top1_match_rate(__x, __x), float)

    def test_value_in_unit_interval(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __r = deformers.pipelines.eval.top1_match_rate(__t, __s)
        assert 0.0 <= __r <= 1.0
...
class TestTopkSetMatchRate:
    ...
    def test_rate_one_on_identical_logits(self):
        __x = torch.randn(_B, _T, _V)
        assert deformers.pipelines.eval.topk_set_match_rate(__x, __x, k=_K) == pytest.approx(1.0)
    ...
    def test_rate_zero_when_sets_disjoint(self):
        ...
        assert deformers.pipelines.eval.topk_set_match_rate(__t, __s, k=_K) == pytest.approx(0.0)
    ...
    def test_ignores_order_within_set(self):
        ...
        assert deformers.pipelines.eval.topk_set_match_rate(__t, __s, k=_K) == pytest.approx(1.0)

    def test_returns_float(self):
        __x = torch.randn(_B, _T, _V)
        assert isinstance(deformers.pipelines.eval.topk_set_match_rate(__x, __x, k=_K), float)

    def test_value_in_unit_interval(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __r = deformers.pipelines.eval.topk_set_match_rate(__t, __s, k=_K)
        assert 0.0 <= __r <= 1.0

    def test_k_equals_one_matches_top1_rate(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __set_rate = deformers.pipelines.eval.topk_set_match_rate(__t, __s, k=1)
        __top1_rate = deformers.pipelines.eval.top1_match_rate(__t, __s)
...
class TestTopkOrderMatchRate:
    ...
    def test_rate_one_on_identical_logits(self):
        __x = torch.randn(_B, _T, _V)
        assert deformers.pipelines.eval.topk_order_match_rate(__x, __x, k=_K) == pytest.approx(1.0)
    ...
    def test_rate_zero_when_sets_disjoint(self):
        ...
        for __i in range(_K):
            __t[:, :, __i] = float(_K - __i)
            __s[:, :, _K + __i] = float(_K - __i)
        assert deformers.pipelines.eval.topk_order_match_rate(__t, __s, k=_K) == pytest.approx(0.0)
    ...
    def test_returns_float(self):
        __x = torch.randn(_B, _T, _V)
        assert isinstance(deformers.pipelines.eval.topk_order_match_rate(__x, __x, k=_K), float)

    def test_value_in_unit_interval(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __r = deformers.pipelines.eval.topk_order_match_rate(__t, __s, k=_K)
        assert 0.0 <= __r <= 1.0
    ...
    def test_order_rate_leq_set_rate(self):
        ...
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __order = deformers.pipelines.eval.topk_order_match_rate(__t, __s, k=_K)
        __set = deformers.pipelines.eval.topk_set_match_rate(__t, __s, k=_K)
        assert __order <= __set + 1e-6

    def test_k_equals_one_matches_top1_rate(self):
        __t = torch.randn(_B, _T, _V)
        __s = torch.randn(_B, _T, _V)
        __order_rate = deformers.pipelines.eval.topk_order_match_rate(__t, __s, k=1)
        __top1_rate = deformers.pipelines.eval.top1_match_rate(__t, __s)
...
class TestBuildVocabProbe:
    ...
    def test_output_shape(self):
        __ids = deformers.pipelines.eval.build_vocab_probe(vocab_size=100, batch_dim=_B, seq_dim=_T)
        assert __ids.shape == (_B, _T)

    def test_values_in_range(self):
        __vocab = 50
        __ids = deformers.pipelines.eval.build_vocab_probe(vocab_size=__vocab, batch_dim=_B, seq_dim=_T)
        assert __ids.min().item() >= 0
        assert __ids.max().item() < __vocab

    def test_deterministic(self):
        __a = deformers.pipelines.eval.build_vocab_probe(vocab_size=100, batch_dim=_B, seq_dim=_T)
        __b = deformers.pipelines.eval.build_vocab_probe(vocab_size=100, batch_dim=_B, seq_dim=_T)
